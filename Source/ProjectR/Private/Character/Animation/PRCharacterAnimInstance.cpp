...
static const FName NAME__PRCharacterAnimInstance__RotationAmount(TEXT("RotationAmount"));
...
static const FName NAME__PRCharacterAnimInstance__root(TEXT("root"));


void UPRCharacterAnimInstance::NativeInitializeAnimation()
{
	Super::NativeInitializeAnimation();
	Character = Cast<APRBaseCharacter>(TryGetPawnOwner());
	if (Character)
	{
		Character->OnJumpedDelegate.AddUniqueDynamic(this, &UPRCharacterAnimInstance::OnJumped);
	}
}

void UPRCharacterAnimInstance::NativeBeginPlay()
{
	...
	// This is the reason why it is tried here to get the PR debug component.
	if (APawn* Owner = TryGetPawnOwner())
	{
		PRDebugComponent = Owner->FindComponentByClass<UPRDebugComponent>();
	}
}

void UPRCharacterAnimInstance::NativeUpdateAnimation(float DeltaSeconds)
{
...
void UPRCharacterAnimInstance::PlayTransition(const FPRDynamicMontageParams& Parameters)
{
...
void UPRCharacterAnimInstance::PlayTransitionChecked(const FPRDynamicMontageParams& Parameters)
{
...
void UPRCharacterAnimInstance::PlayDynamicTransition(float ReTriggerDelay, FPRDynamicMontageParams Parameters)
{
	if (bCanPlayDynamicTransition)
	{
		...
		World->GetTimerManager().SetTimer(PlayDynamicTransitionTimer, this,
		                                  &UPRCharacterAnimInstance::PlayDynamicTransitionDelay,
		                                  ReTriggerDelay, false);
	}
}

bool UPRCharacterAnimInstance::ShouldMoveCheck() const
{
	return (CharacterInformation.bIsMoving && CharacterInformation.bHasMovementInput) ||
		CharacterInformation.Speed > 150.0f;
}

bool UPRCharacterAnimInstance::CanRotateInPlace() const
{
	return RotationMode.Aiming() ||
		CharacterInformation.ViewMode == EPRViewMode::FirstPerson;
}

bool UPRCharacterAnimInstance::CanTurnInPlace() const
{
	return RotationMode.LookingDirection() &&
		CharacterInformation.ViewMode == EPRViewMode::ThirdPerson &&
		GetCurveValue(NAME_Enable_Transition) >= 0.99f;
}

bool UPRCharacterAnimInstance::CanDynamicTransition() const
{
	return GetCurveValue(NAME_Enable_Transition) >= 0.99f;
}

void UPRCharacterAnimInstance::PlayDynamicTransitionDelay()
{
	bCanPlayDynamicTransition = true;
}

void UPRCharacterAnimInstance::OnJumpedDelay()
{
	InAir.bJumped = false;
}

void UPRCharacterAnimInstance::OnPivotDelay()
{
	Grounded.bPivot = false;
}

void UPRCharacterAnimInstance::UpdateAimingValues(float DeltaSeconds)
{
...
void UPRCharacterAnimInstance::UpdateLayerValues()
{
...
void UPRCharacterAnimInstance::UpdateFootIK(float DeltaSeconds)
{
	...
	else if (!MovementState.Ragdoll())
	{
		...
		SetFootOffsets(DeltaSeconds, NAME_Enable_FootIK_L, IkFootL_BoneName, NAME__PRCharacterAnimInstance__root,
		               FootOffsetLTarget,
		               FootIKValues.FootOffset_L_Location, FootIKValues.FootOffset_L_Rotation);
		SetFootOffsets(DeltaSeconds, NAME_Enable_FootIK_R, IkFootR_BoneName, NAME__PRCharacterAnimInstance__root,
		               FootOffsetRTarget,
...
void UPRCharacterAnimInstance::SetFootLocking(float DeltaSeconds, FName EnableFootIKCurve, FName FootLockCurve,
                                               ...
                                               FVector& CurFootLockLoc, FRotator& CurFootLockRot)
{
	...
	if (UseFootLockCurve)
	{
		UseFootLockCurve = FMath::Abs(GetCurveValue(NAME__PRCharacterAnimInstance__RotationAmount)) <= 0.001f ||
...
void UPRCharacterAnimInstance::SetFootLockOffsets(float DeltaSeconds, FVector& LocalLoc, FRotator& LocalRot)
{
...
void UPRCharacterAnimInstance::SetPelvisIKOffset(float DeltaSeconds, FVector FootOffsetLTarget,
...
void UPRCharacterAnimInstance::ResetIKOffsets(float DeltaSeconds)
{
...
void UPRCharacterAnimInstance::SetFootOffsets(float DeltaSeconds, FName EnableFootIKCurve, FName IKFootBone,
                                               ...
                                               FRotator& CurRotationOffset)
{
	...
	{
		UPRDebugComponent::DrawDebugLineTraceSingle(
			World,
			TraceStart,
			TraceEnd,
...
void UPRCharacterAnimInstance::RotateInPlaceCheck()
{
...
void UPRCharacterAnimInstance::TurnInPlaceCheck(float DeltaSeconds)
{
...
void UPRCharacterAnimInstance::DynamicTransitionCheck()
{
	...
	{
		FPRDynamicMontageParams Params;
...
void UPRCharacterAnimInstance::UpdateMovementValues(float DeltaSeconds)
{
	// Interp and set the Velocity Blend.
	const FPRVelocityBlend& TargetBlend = CalculateVelocityBlend();
...
void UPRCharacterAnimInstance::UpdateRotationValues()
{
...
void UPRCharacterAnimInstance::UpdateInAirValues(float DeltaSeconds)
{
	...
	const FPRLeanAmount& InAirLeanAmount = CalculateAirLeanAmount();
...
void UPRCharacterAnimInstance::UpdateRagdollValues()
{
	// Scale the Flail Rate by the velocity length. The faster the ragdoll moves, the faster the character will flail.
	const float VelocityLength = GetOwningComponent()->GetPhysicsLinearVelocity(NAME__PRCharacterAnimInstance__root).Size();
	FlailRate = FMath::GetMappedRangeValueClamped<float, float>({0.0f, 1000.0f}, {0.0f, 1.0f}, VelocityLength);
}

float UPRCharacterAnimInstance::GetAnimCurveClamped(const FName& Name, float Bias, float ClampMin,
                                                     float ClampMax) const
{
	return FMath::Clamp(GetCurveValue(Name) + Bias, ClampMin, ClampMax);
}

FPRVelocityBlend UPRCharacterAnimInstance::CalculateVelocityBlend() const
{
...
FVector UPRCharacterAnimInstance::CalculateRelativeAccelerationAmount() const
{
...
float UPRCharacterAnimInstance::CalculateStrideBlend() const
{
...
float UPRCharacterAnimInstance::CalculateWalkRunBlend() const
{
...
float UPRCharacterAnimInstance::CalculateStandingPlayRate() const
{
...
float UPRCharacterAnimInstance::CalculateDiagonalScaleAmount() const
{
...
float UPRCharacterAnimInstance::CalculateCrouchingPlayRate() const
{
...
float UPRCharacterAnimInstance::CalculateLandPrediction() const
{
...
FPRLeanAmount UPRCharacterAnimInstance::CalculateAirLeanAmount() const
{
...
EPRMovementDirection UPRCharacterAnimInstance::CalculateMovementDirection() const
{
	...
	FRotator Delta = CharacterInformation.Velocity.ToOrientationRotator() - CharacterInformation.AimingRotation;
	Delta.Normalize();
	return UPRMathLibrary::CalculateQuadrant(MovementDirection, 70.0f, -70.0f, 110.0f, -110.0f, 5.0f, Delta.Yaw);
}

void UPRCharacterAnimInstance::TurnInPlace(FRotator TargetRotation, float PlayRateScale, float StartTime,
                                            bool OverrideCurrent)
{
	...
	FPRTurnInPlaceAsset TargetTurnAsset;
	if (Stance.Standing())
	{
...
void UPRCharacterAnimInstance::OnJumped()
{
	InAir.bJumped = true;
	...
	GetWorld()->GetTimerManager().SetTimer(OnJumpedTimer, this,
	                                  &UPRCharacterAnimInstance::OnJumpedDelay, 0.1f, false);
}

void UPRCharacterAnimInstance::OnPivot()
{
