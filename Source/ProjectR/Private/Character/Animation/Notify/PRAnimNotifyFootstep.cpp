#include "Character/Animation/Notify/PRAnimNotifyFootstep.h"
...
FName UPRAnimNotifyFootstep::NAME_FootstepType(TEXT("FootstepType"));
FName UPRAnimNotifyFootstep::NAME_Foot_R(TEXT("Foot_R"));


void UPRAnimNotifyFootstep::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation, const FAnimNotifyEventReference& EventReference)
{
	...
	if (HitDataTable)
	{
		                                          ...
		                                          DrawDebugType, Hit, true /*bIgnoreSelf*/))
		{
			...
			checkNoRecursion();
			static TArray<FPRHitFX*> HitFXRows;
			HitFXRows.Reset();

			HitDataTable->GetAllRows<FPRHitFX>(FString(), HitFXRows);

			FPRHitFX* HitFX = nullptr;
			if (auto FoundResult = HitFXRows.FindByPredicate([&](const FPRHitFX* Value)
			{
			...
			else if (auto DefaultResult = HitFXRows.FindByPredicate([&](const FPRHitFX* Value)
			{
...
FString UPRAnimNotifyFootstep::GetNotifyName_Implementation() const
{
