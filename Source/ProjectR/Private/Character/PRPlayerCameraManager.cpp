...
APRPlayerCameraManager::APRPlayerCameraManager()
{
...
void APRPlayerCameraManager::OnPossess(APRBaseCharacter* NewCharacter)
{
	...
	UPRPlayerCameraBehavior* CastedBehv = Cast<UPRPlayerCameraBehavior>(CameraBehavior->GetAnimInstance());
	if (CastedBehv)
	{
	...
	PRDebugComponent = ControlledCharacter->FindComponentByClass<UPRDebugComponent>();
}

float APRPlayerCameraManager::GetCameraBehaviorParam(FName CurveName) const
{
...
void APRPlayerCameraManager::UpdateViewTargetInternal(FTViewTarget& OutVT, float DeltaTime)
{
	...
	if (OutVT.Target)
	{
		...
		if (OutVT.Target->IsA<APRBaseCharacter>())
		{
...
FVector APRPlayerCameraManager::CalculateAxisIndependentLag(FVector CurrentLocation, FVector TargetLocation,
...
bool APRPlayerCameraManager::CustomCameraBehavior(float DeltaTime, FVector& Location, FRotator& Rotation, float& FOV)
{
